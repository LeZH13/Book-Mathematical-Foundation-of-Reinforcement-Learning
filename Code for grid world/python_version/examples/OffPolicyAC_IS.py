...
def behavior_policy(probs, epsilon=0.2):
...
for episode in range(500):
    ...
    while not done:
        if render_this_episode:
            env.render()
        target_probs, h_actor = actor.forward(obs)
        # Get behavior policy probabilities
        epsilon = 0.2
        behavior_probs = (1 - epsilon) * target_probs + epsilon / act_dim
        action = behavior_policy(target_probs, epsilon=epsilon)
        ...
        advantage = reward + gamma * next_value * (not done) - value

        # Importance sampling ratio
        rho = target_probs[action] / behavior_probs[action]

        # Actor update (policy gradient with importance sampling)
        dlog = -target_probs
        dlog[action] += 1
        actor_grads = {
            'w2': np.outer(h_actor, dlog) * advantage * rho,
            'w1': np.outer(obs, np.dot(actor.w2, dlog) * (1 - h_actor ** 2) * advantage * rho)
        }
        actor.update(actor_grads)

        # Critic update (TD error, on-policy)
        critic_grads = {
            'w2': h_critic[:, None] * advantage,
            'w1': np.outer(obs, np.dot(critic.w2.flatten(), advantage) * (1 - h_critic ** 2))
        }
...
plt.title('Off-Policy Actor-Critic (IS): Episode Reward Progress')
plt.show()